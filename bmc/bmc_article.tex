...
model, widely adopted in large-scale systems. By its turn, the less popular
...
that already implemented eventual consistency \cite{Riak2013}. Besides the new
...
\cite{DeDiana2013a, DeDiana2013b}.
...
homogeneous hardware. The environment is not virtualized (barebone servers)
and is monitored and highly configurable, so researchers have great control of
the experiments.
...
\begin{figure}[h!] \caption{Boxplot of response times for maximum load per thread.}
...
percentile for example \cite{DeCandia2007}, per-record timeline consistency
is not appropriate.
