...
experimental results confirm that a system under per-object timeline
consistency suffer a low performance hit compared to the same system under
eventual consistency if it exhibits high data locality and read/write ratios.
This comparative presents resources for storage system developers and
applications built over them about the viability of a consistency moderl other
than eventual consistency for large-scale systems.
%\parttitle{First part title} %if any Text for this section.
...
actions to correct results of actions taken based on inconsistent states. The
...
Per-object timeline consistency stays in a middle ground between
...
for web applications where users tolerate some level of unavailability.
...
developers of large-scale storage systems and the applications that use them.
...
Some web applications cannot tolerate conflicts, such as an auction site, where
the bidding history of a product must be unique to all users at all times. In
a system under eventual consistency, in case of a failure that splits the
network in two or more partitions, users acting in each partition will have
their own view of the bidding history, what is equivalent to two or more
simultaneous auctions for the same item.
...
replica's data center incur the communication cost over a WAN. However, the
...
writes, network communication costs are low on average, especially if reads do
not necessarily need the latest value. The PNUTS developers implemented a
simple heuristic by which the master replica migrates to the data center that
...
remote or vice versa. Thus, the proportions of reads and writes for each mode
were:
...
January 2013, it shows an average latency of 238.06~ms with standard deviation
143.00~ms, resulting in a jitter of 60\%. The previous 11 months show similar
...
them. The only system implementing per-object timeline consistency is PNUTS,
